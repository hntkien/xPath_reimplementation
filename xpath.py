...
class xPath_Explainer:
	...
	def get_proxy_graph(self, g, mptp, p):
		...
		x = {tp: g.ndata["feat"][tp].clone().detach().cpu() for tp in g.ntypes}
		sg_n = {n: g.nodes[n].data['feat'].shape[0] for n in g.ntypes}
		proxy_ids = [-1]
		...
		for etype in g.canonical_etypes:
			stp, etp, ttp = etype
			sg_edges[(stp, etp, ttp)] = [g.edges(etype=etype)[0].tolist(), g.edges(etype=etype)[1].tolist()]
		...
		for stp, etp, dtp in g.canonical_etypes:
			...
			if stp == mptp[-1] and dtp == mptp[-2]:
				...
				print(f"Attempting to pop index {del_id} for edge type {etype}")
				if 0 <= del_id < len(sg_edges[etype][0]) and 0 <= del_id < len(sg_edges[etype][1]):
					sg_edges[etype][0].pop(del_id)
					sg_edges[etype][1].pop(del_id)
				else:
					print(f"Invalid pop index: {del_id} for edge type {etype}. List lengths: {len(sg_edges[etype][0])}, {len(sg_edges[etype][1])}")
	...
	def explain_beam(self, g, node_list, beam=3, sample_n=10):
		...
		for neighbors, _, _ in tqdm(subgraph_dataloader):
			...
			x = {tp: subgraph.ndata["feat"][tp].clone() for tp in subgraph.ntypes}
